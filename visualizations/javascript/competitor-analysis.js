...
async function getIssuesForRepo(repo, page = 1){
	return apiRequest(`https://api.github.com/repos/${repo}/issues?page=${page}&per_page=100`);
}
...
async function getGithubRepoName(packageName){
	let { repository: { url } } = await apiRequest(`https://registry.npmjs.org/${packageName}`);
...
async function getCompetitorAnalysis(packageName){
	...
	return {
		starCount: await getStargazersForRepo(repoName).then((stargazers) => stargazers.length),
...
function calculateMetrics(issueOrPrs){
	...
	for(let { title, reactions } of issueOrPrs){
		for(let reaction in reactions){
			if(reaction == "total_count" || reaction == "url" || reactions[reaction] == 0)
				continue;
			if(!(reaction in totalReactions))
				totalReactions[reaction] = 0;
...
getCompetitorAnalysis("axios").then(console.table);